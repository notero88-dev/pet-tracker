...
class Mode8ConfigurationController {
  ...
  Future<Mode8WizardOutcome> run(BuildContext context) async {
    ...
    // 2026-07-28 (Lote 1): the client-side Traccar geofence create that
    // used to live here was REMOVED. The backend's homeSetupRunner
    // already creates and links the home geofence server-side; the
    // duplicate client create was the source of the multiplied
    // "X — Casa" zones observed in prod (4 copies for one device).
    // We keep the one thing it actually bought us — the map showing the
    // zone immediately — by refreshing the geofence list instead.
    // (The legacy A6 path that consumed the returned geofenceId is dead
    // code; live consumers ignore the id.)
    final traccar = Provider.of<TraccarProvider>(context, listen: false);
    await traccar.loadGeofences();

    _emit(Mode8WizardState.success);
    return const Mode8WizardSuccess(traccarGeofenceId: 0);
  }
